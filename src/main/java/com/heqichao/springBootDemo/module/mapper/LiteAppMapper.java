...
public interface LiteAppMapper {
	...
	@Insert("insert into applications (app_id,app_name,secret,platform_ip,app_auth,callback_url,post_asyn_cmd,subscribe_notifycation,remark,own_id,status,update_uid)"
	...
	@Update("update  applications set app_id=#{appId},app_name=#{appName},"
	...
	@Update("update  applications set secret=#{secret},update_time=sysdate(),update_uid=#{updateUid}"
			+ " where id=#{id} and status = 'N' ")
	public int resetAppSecret(LiteApplication app);

	@Select("select count(1)>0 from applications where app_id = #{appId} and status = 'N' ")
	public boolean duplicatedAid(@Param("appId")String appId);
	
	@Select("select count(1)>0 from applications where app_id = #{appId} and status = 'N' and id not in (#{id}) ")
	public boolean duplicatedEidByUpd(@Param("appId")String appId,@Param("id")Integer id);


    @Delete("update applications set status = 'D',update_time=sysdate(),update_uid=#{uid} where status = 'N' and id= #{id}   ")
